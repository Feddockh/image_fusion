...
# ros2 run multi_cam_rig_cpp director_time_synchronizer --ros-args -p use_sim_time:=True --demosaic:=True
    ...
    def save_request_images(self, request: DirectorRequest):
        ...
        for cam_name, cv_img in request.images_received.items():
            ...
            if self.demosaic and cam_name.lower() == "ximea":
                try:
                    hypercube = demosaic_ximea_5x5(cv_img)
                except Exception as e:
                    self.get_logger().error(f"Demosaicing failed for {cam_name}: {e}")
                    continue

                for bandwidth, band_img in hypercube.items():
                    band_dir = os.path.join(self.output_dir, f"{cam_name}_{bandwidth}")
                    os.makedirs(band_dir, exist_ok=True)
                    filename = f"{stamp_str}_{cam_name}_{bandwidth}.tif"
